...
- **Project Skills Only**: Only use project-specific skills defined in `.agents/skills/` (`analyze-status`, `propose-next-steps`, `ai-settings-update`, `import-from-jira`, `answer-question`, `compute-metrics`, `ingest-jira`, `seed-jira`) or built-in system tools.
