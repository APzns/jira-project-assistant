...
- **Preserve Unrelated Layout**: NEVER modify, remove, or restructure HTML elements, CSS classes, or UI layout text that are not explicitly related to the currently discussed change. Ensure collateral damage (such as replacing headers or removing labels) is strictly avoided when editing UI files.
